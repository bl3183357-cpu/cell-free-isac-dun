...
def train_isac_model():
    ...
    # 【修改点 1】：定义固定的数据集大小
    TOTAL_SAMPLES = 50000  # 总共生成 5 万个信道样本
    BATCH_SIZE = 1024
    EPOCHS = 50
    LR = 1e-3
    ALPHA, RATE_NORM, SENSE_NORM = 0.3, 1.0, 10.0
    ...
    H_all = generate_rayleigh_channel(TOTAL_SAMPLES, K, NUM_TX, device='cpu')
    a_all = generate_steering_vector(TOTAL_SAMPLES, NUM_TX, device='cpu')
    ...
    model = ISAC_GNN_UnfoldingNet(
        num_ap=M, antennas_per_ap=N, num_layers=8, hidden_dim=64, p_max=P_MAX
    ).to(device)
